<?php
      ...
      function render_buttons_group ( $buttons_group ) {
        $buttons = $buttons_group['button_set'];
        $flex_direction = $buttons_group['stacked'] ? "d-flex flex-column col-6" : "d-flex flex-row";

        $output = "<div class=' {$flex_direction} ' >";
      ...
      function render_text_content( $title, $subtitle, $text, $class="col-sm-12 col-md-6", $group=[]) {
          return "<div class=' $class '>".
                  ($title ? "<h1>$title</h1>" : "").
                  ($subtitle ? "<h2>$subtitle</h2>" : "").
                  ($text ? "<p>$text</p>" : "").
                  render_buttons_group($group).
          "</div>";
      };

      function render_text_field($text) { // receive argument as provided by the acf_widget
        return "
      ...
      function render_image($imgArr) {
          if ($imgArr) {
              ...
              return "<picture>
                ...
                <img src='$thumbnail' alt='$title' style='objetc-fit: cover; width: 100%; height: 100%'>
              </picture>";
          }
      }
      ...
      function render_cta_field ( $options, $text, $media, $buttons ){
        ...
        $bgcolor   = $options['background']['solid_color'];
        $bgfrom    = $options['background']['gradient_from'];
        $bgto      = $options['background']['gradient_to'];
        ...
        if ($options['content_position'] == "background") {
         $output = "
          <section class='call-to-action' type=$bgtype style='--bgcolor: $bgcolor; --bgfrom: $bgfrom; --bgto: $bgto'>
            <div class='container-".$options['width']." h-100 content-position-background'>
              <div class='row'>
                  <div class='background-content' >".
                    ( $media['content_type'] == 'image'     ? render_image($media['image']) : "" ).
                    ( $media['content_type'] == 'video'     ? render_video($media['video'], $media['poster']) : "" ).
                    ( $media['content_type'] == 'animation' ? render_lottie($media['animation']) : "" ).
                    ( $media['content_type'] == '3d'        ? render_3d($media['three_d'], $media['poster3d']) : "" ).
                    ( $media['content_type'] == 'embeded'   ? render_embeded($media['embeded']) : "" )."
                  </div>
                  <div class='overlay' style='--overshow: $overshow; --overopac: $overopac; --overcolor: $overcolor'></div>
                  <div class='info col-sm-11 col-md-9 col-lg-8 col-xl-6' center='$center' >
                    <div class='txt'>".render_text_field($text)."</div>
                    <div class='buttons'>".render_buttons_group($buttons)."</div>
                  </div>
                </div>
              </div>
          </section>";
        ...
        if ($options['content_position'] == "left") {
         ...
         if ($options['content_position'] == "left-border") {
          $output = "
           <section class='call-to-action' type=$bgtype style='--bgcolor: $bgcolor; --bgfrom: $bgfrom; --bgto: $bgto'>
             <div class='container-".$options['width']." h-100 content-position-background'>
                <div class='row'>
                   <div class='background-content-at-left-border' >".
                     ( $media['content_type'] == 'image'     ? render_image($media['image']) : "" ).
                     ( $media['content_type'] == 'video'     ? render_video($media['video'], $media['poster']) : "" ).
                     ( $media['content_type'] == 'animation' ? render_lottie($media['animation']) : "" ).
                     ( $media['content_type'] == '3d'        ? render_3d($media['three_d'], $media['poster3d']) : "" ).
         ...
         if ($options['content_position'] == "right-border") {
          $output = "
           <section class='call-to-action' type=$bgtype style='--bgcolor: $bgcolor; --bgfrom: $bgfrom; --bgto: $bgto'>
             <div class='container-".$options['width']." h-100 content-position-background'>
                   <div class='background-content-at-right-border'>".
                     ( $media['content_type'] == 'image'     ? render_image($media['image']) : "" ).
                     ( $media['content_type'] == 'video'     ? render_video($media['video'], $media['poster']) : "" ).
                     ( $media['content_type'] == 'animation' ? render_lottie($media['animation']) : "" ).
                     ( $media['content_type'] == '3d'        ? render_3d($media['three_d'], $media['poster3d']) : "" ).
